...
fn export_dry_run_is_side_effect_free_and_execution_uses_manifest_paths() {
    ...
    let dry_run = export(&input, &output_root, true);
    ...
    let execution = export(&input, &output_root, false);
    assert_success(&execution);
    let manifest: serde_json::Value = serde_json::from_slice(&execution.stdout).unwrap();
    assert_eq!(manifest["contract"], "unity_asset.extraction_manifest");
...
#[test]
fn help_exposes_one_export_command_and_no_legacy_export_commands() {
    let output = run(["--help"]);
...
fn export_resume_reuses_verified_outputs_across_independent_processes() {
    ...
    let first = export(&input, &output_root, false);
    assert_success(&first);
    fs::write(&manifest_path, &first.stdout).unwrap();

    let resumed = export_with_resume(&input, &output_root, &manifest_path);
    assert_success(&resumed);
    let manifest: serde_json::Value = serde_json::from_slice(&resumed.stdout).unwrap();
    let artifacts = manifest["artifacts"].as_array().unwrap();
    assert_eq!(artifacts.len(), 1);
...
fn export_atomically_publishes_a_durable_resume_manifest() {
    ...
    assert_success(&resumed);
    let manifest: serde_json::Value = serde_json::from_slice(&resumed.stdout).unwrap();
    assert_eq!(manifest["artifacts"][0]["status"], "resumed");
    assert_eq!(fs::read(&manifest_path).unwrap(), resumed.stdout);
...
fn export_plan_is_replayable_without_replanning_and_can_resume() {
    ...
    let manifest: serde_json::Value = serde_json::from_slice(&executed.stdout).unwrap();
    assert_eq!(manifest["artifacts"][0]["status"], "written");
    fs::write(&manifest_path, &executed.stdout).unwrap();
...
fn export(input: &Path, output: &Path, dry_run: bool) -> Output {
    export_with_inputs(input, output, dry_run, None, None, None)
}

fn export_with_resume(input: &Path, output: &Path, resume: &Path) -> Output {
    export_with_inputs(input, output, false, Some(resume), None, None)
}

fn export_with_plan(input: &Path, output: &Path, plan: &Path, resume: Option<&Path>) -> Output {
    export_with_inputs(input, output, false, resume, Some(plan), None)
}
...
fn export_with_manifest(
    ...
    export_with_inputs(input, output, false, resume, None, Some(manifest))
}
...
fn export_with_inputs(
    ...
    command
        ...
        .arg(output);
    if plan.is_none() {
        command.arg("--class-id").arg("28").arg("--limit").arg("1");
    }
    if dry_run {
        command.arg("--dry-run");
    }
