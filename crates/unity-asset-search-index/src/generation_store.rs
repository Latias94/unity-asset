...
use crate::generation::{
    ArtifactTreeEvidence, GenerationArtifactEvidence,
    LEGACY_COUPLED_GENERATION_STORAGE_CONTRACT_VERSION, SEARCH_GENERATION_STORAGE_CONTRACT_VERSION,
...
pub(crate) const SOURCE_STATE_FILE: &str = "source-state-v5.json";
...
const OBSOLETE_GENERATION_DIRECTORY_PREFIXES: [&str; 4] = [
    "generation-v1-",
    "generation-v2-",
    "generation-v3-",
    "generation-v4-",
...
enum GenerationStorageLayout {
    RebuildOnly,
    OpaqueProjection,
    CurrentSourceStateV5,
}

impl GenerationStorageLayout {
    fn from_contract(contract: u16) -> Option<Self> {
        match contract {
            1 | 2 => Some(Self::RebuildOnly),
            3 | LEGACY_COUPLED_GENERATION_STORAGE_CONTRACT_VERSION => Some(Self::OpaqueProjection),
            SEARCH_GENERATION_STORAGE_CONTRACT_VERSION => Some(Self::CurrentSourceStateV5),
            _ => None,
        }
    }
}
...
impl GenerationSnapshot {
    ...
    #[must_use]
    pub(crate) const fn storage_contract(&self) -> u16 {
        self.storage_contract
    }

    #[must_use]
    pub(crate) const fn source_state_storage_current(&self) -> bool {
        self.storage_contract == SEARCH_GENERATION_STORAGE_CONTRACT_VERSION
    }
    ...
    pub(crate) fn load_source_state(
        ...
        if !self.source_state_storage_current() {
            return Err(GenerationStoreError::ObsoleteSourceStateStorage {
                actual: self.storage_contract,
...
/// The durable result of appending a desired-revision head.
///
/// The variants encode the only states the store can produce: there may be no active generation,
/// the existing head may already have the requested desired revision, or a new durable head may
/// have been committed. Callers must install a committed report before releasing their public
/// observation lock; they must not reconstruct freshness from the requested revision alone.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum DesiredRevisionCommit {
    NoActive,
    Unchanged,
    Committed(Box<GenerationPublishReport>),
}
...
pub(crate) enum GenerationFailpoint {
    ...
    #[cfg(test)]
    StartupActivationDirectorySync,
    #[cfg(test)]
...
pub(crate) struct GenerationStagingRecoveryReport {
    removed_entries: u64,
    activation_authority_changed: bool,
}
...
impl GenerationStagingRecoveryReport {
    ...
    #[must_use]
    const fn activation_authority_changed(self) -> bool {
        self.activation_authority_changed
    }
}
...
fn merge_recovery_results(
    ...
    match (first, second) {
        (Ok(first), Ok(second)) => Ok(GenerationStagingRecoveryReport {
            ...
            activation_authority_changed: first.activation_authority_changed
                || second.activation_authority_changed,
        }),
        (Err(error), _) | (Ok(_), Err(error)) => Err(error),
    }
}

fn activation_recovery_requires_reopen(error: GenerationStoreError) -> GenerationStoreError {
    if error.requires_reopen() {
        error
    } else {
...
pub(crate) struct IndexRebuildRequired {
    ...
    manifest_digest: DigestV1,
    parent_generation: Option<SearchGenerationId>,
    bootstrap: GenerationRebuildBootstrap,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct GenerationRebuildBootstrap {
...
impl GenerationRebuildBootstrap {
    #[must_use]
    ...
    #[must_use]
    pub(crate) const fn actual_revision(&self) -> WorkspaceRevision {
        self.actual_revision
    }

    #[must_use]
    pub(crate) const fn desired_revision(&self) -> WorkspaceRevision {
        self.desired_revision
    }
...
impl IndexRebuildRequired {
    #[must_use]
    pub(crate) const fn bootstrap(&self) -> &GenerationRebuildBootstrap {
        &self.bootstrap
    }
    ...
    #[must_use]
    pub(crate) const fn manifest_digest(&self) -> DigestV1 {
...
pub(crate) enum GenerationStartupDisposition {
    Ready,
    RebuildRequired(Box<IndexRebuildRequired>),
}

impl GenerationStartupDisposition {
    #[must_use]
    pub(crate) fn rebuild_required(&self) -> Option<&IndexRebuildRequired> {
        match self {
            Self::Ready => None,
            Self::RebuildRequired(required) => Some(required.as_ref()),
        }
    }
}
...
///
/// Staging is not authoritative for the active generation. Ordinary cleanup failures are reported
/// separately, but a residue that remains hard-linked to a committed head still violates the
/// persisted single-link identity contract and fails closed while that head is opened.
#[derive(Debug)]
...
impl GenerationStore {
    ...
    /// share the caller's ledger; abandoned-staging cleanup uses an independent bounded ledger.
    /// Cleanup failures remain maintenance evidence unless the residue violates a committed
    /// head's persisted identity contract.
    ...
    fn open_at_root(
        ...
        // authorities. Resolve committed activation authorities before opening committed heads;
        // an unresolved authority is never downgraded to maintenance because it can change which
        // head is authoritative. Disposable cleanup is intentionally handled as a separate,
        // degradable pass below.
        let mut recovery_budget = AssetLoadBudget::default();
        let committed_recovery = recover_committed_activation_authority(
            &staging,
            &activations,
            &mut recovery_budget,
            #[cfg(test)]
        ...
        let disposable_recovery = match recover_disposable_staging(
            &staging,
            &activations,
            &mut recovery_budget,
            #[cfg(test)]
            ...
            Err(error) if error.requires_reopen() => return Err(error),
            recovery => recovery,
        };
        let mut staging_recovery =
            merge_recovery_results(Ok(committed_recovery), disposable_recovery);
        ...
        let activation_snapshot =
            activation_candidates_for_open(&activations, &opened_activations, budget)?;
        ...
        let (active, startup_disposition) = match selected {
            Ok(SelectedGeneration::Ready(active)) => (active, GenerationStartupDisposition::Ready),
            Ok(SelectedGeneration::RebuildRequired { active, required }) => (
                active,
                GenerationStartupDisposition::RebuildRequired(required),
            ),
            Err(error) => return Err(error),
        };
        let protected_generation = active.as_ref().map(|snapshot| snapshot.directory.clone());
    ...
    pub(crate) fn reconcile_abandoned_staging(
        ...
        self.reconcile_abandoned_staging_inner(
            budget,
            #[cfg(test)]
            None,
        )
    ...
    pub(crate) fn reconcile_abandoned_staging_with_failpoint(
        ...
        self.reconcile_abandoned_staging_inner(budget, Some(failpoint))
    }

    fn reconcile_abandoned_staging_inner(
        &mut self,
        budget: &mut AssetLoadBudget,
        ...
        self.revalidate_private_root(
            "revalidate private index root before durable staging reconciliation",
        )?;
        let authority_report = match recover_committed_activation_authority(
            &self.staging,
            &self.activations,
            budget,
            #[cfg(test)]
        ...
        if authority_report.activation_authority_changed() {
            // The recovery pass may have changed which durable activation is authoritative. The
            // current store instance no longer has a proof that its in-memory handles match that
            // authority; force callers to reopen before any further operation can proceed.
            self.activation_outcome_unknown = true;
            return Err(GenerationStoreError::ActivationAuthorityChangedReopenRequired);
        }
        let disposable_report = match recover_disposable_staging(
            &self.staging,
            &self.activations,
            budget,
            #[cfg(test)]
        ...
        if disposable_report.activation_authority_changed() {
            self.activation_outcome_unknown = true;
            return Err(GenerationStoreError::ActivationAuthorityChangedReopenRequired);
        }
        Ok(disposable_report)
    }
    ...
    fn record_desired_revision_with_failpoint(
        ...
        let Some(active) = self.active.as_ref() else {
            return Ok(DesiredRevisionCommit::NoActive);
        };
        if active.desired_revision == desired_revision {
            return Ok(DesiredRevisionCommit::Unchanged);
        }
        ...
        let manifest_digest = updated.manifest_digest;
        let report =
            self.activate_prepared(updated, manifest_digest, Vec::new(), failpoint, budget)?;
        Ok(DesiredRevisionCommit::Committed(Box::new(report)))
    }
    ...
    pub fn estimate_publish(
        &self,
        incoming_generation: SearchGenerationId,
        ...
        let old_active_generation_bytes = self
            ...
            .and_then(|active| {
                existing_generations
                    .iter()
                    .find(|(directory, _)| directory == active.directory())
                    .map(|(_, bytes)| *bytes)
            })
            .unwrap_or(0);

        let historical = self.retained_historical_snapshots(budget)?;
        let retained_historical_bytes = if self.options.retain_previous_generations == 0 {
            0
        } else {
            checked_sum(
                self.active
                    ...
                    .filter(|snapshot| snapshot.generation() != incoming_generation)
                    .take(self.options.retain_previous_generations)
                    .map(GenerationSnapshot::directory)
    ...
    pub fn estimate_manifest_publish(
        ...
        self.estimate_publish(manifest.generation_id(), new_generation_bytes, budget)
    }
    ...
    fn prepare_publish_inner(
        ...
        if let Some(active) = self
            ...
            .as_ref()
            .filter(|active| {
                active.generation == generation && active.source_state_storage_current()
            })
            .cloned()
        {
            let completed = inspect_completed_generation(&active.directory, generation, budget)?;
            validate_persisted_source_state(&active.directory, &completed.manifest, budget)?;
        ...
        let replace_invalid_completed = if path_exists_no_follow(&completed_directory)? {
            match inspect_completed_generation(&completed_directory, generation, budget).and_then(
                |completed| {
                    validate_persisted_source_state(
                        &completed_directory,
                        &completed.manifest,
                        budget,
                    )?;
                ...
                Ok(completed) => {
                    ...
                    let snapshot = GenerationSnapshot {
                        activation_ordinal,
                        generation,
                        storage_contract: SEARCH_GENERATION_STORAGE_CONTRACT_VERSION,
                        manifest_digest: completed.manifest_digest,
                        desired_revision,
    ...
    fn activate_prepared(
        ...
        let record = GenerationHeadRecord {
            ...
            generation_storage_contract: snapshot.storage_contract(),
            manifest_digest,
    ...
    pub(crate) fn ensure_operational(&self) -> Result<(), GenerationStoreError> {
    ...
    #[cfg(test)]
    pub(crate) fn poison_activation_outcome_for_test(&mut self) {
        self.activation_outcome_unknown = true;
    }
    ...
    fn write_activation(
        ...
        // The recovery authority is already durable here, but the final activation directory is
        // not yet known to survive a crash. The caller must reopen if that sync cannot be
        // confirmed; continuing with an in-memory head would permit a later crash to roll public
        // freshness back to the previously durable head.
        let mut warnings = Vec::new();
        inject_failure(failpoint, GenerationFailpoint::ActivationDirectorySync)
            .and_then(|()| sync_directory(&self.activations))
            .map_err(
                |source| GenerationStoreError::ActivationCommitRequiresReopen {
        ...
        Ok(ActivationWriteOutcome { warnings })
    }
    ...
    fn prune_retention(&self, budget: &mut AssetLoadBudget) -> Result<(), GenerationStoreError> {
        ...
        visit_directory_entries_budgeted(&self.generations, budget, |entry, budget| {
            ...
            if SearchGenerationId::from_directory_name(name).is_none()
                && !is_obsolete_generation_directory_name(name)
            {
                return Ok(());
            }
...
#[derive(Debug)]
enum SelectedGeneration {
    Ready(Option<GenerationSnapshot>),
    RebuildRequired {
        active: Option<GenerationSnapshot>,
        required: Box<IndexRebuildRequired>,
    },
}
...
#[derive(Debug)]
struct CommittedActivationRecoveryEntry {
    ordinal: u64,
    path: PathBuf,
    file_name: OsString,
}

fn recover_committed_activation_authority(
    staging: &Path,
    activations: &Path,
    budget: &mut AssetLoadBudget,
    #[cfg(test)] failpoint: Option<GenerationFailpoint>,
    ...
    visit_directory_entries_budgeted(staging, budget, |entry, budget| {
        ...
        let Some(ordinal) = parse_activation_recovery_file_name(name) else {
            return Ok(());
        };
        let metadata = metadata_no_follow(&entry.path)?;
        if !metadata.is_file() {
        ...
        reserve_artifact_vec(
            &mut recoveries,
            1,
            "activation recovery authorities",
            budget,
        )?;
        recoveries.push(CommittedActivationRecoveryEntry {
            ordinal,
            path: entry.path,
    ...
    if recoveries.is_empty() {
        return Ok(GenerationStagingRecoveryReport::default());
    }
    let recovery = (|| {
        recoveries.sort_unstable_by_key(|entry| entry.ordinal);

        let mut removed_entries = 0_u64;
        for recovery in &recoveries {
            let pending_path = staging.join(activation_pending_file_name(recovery.ordinal));
            ...
            let final_path = activations.join(activation_file_name(recovery.ordinal));
            if !path_exists_no_follow(&final_path)? {
                fs::hard_link(&recovery.path, &final_path).map_err(|source| {
                    GenerationStoreError::io(
        ...
        // Recovery authority remains durable until every reconstructed or surviving final link is
        // confirmed in the activation directory. Only then may the staging aliases be removed.
        ...
        for recovery in recoveries {
            ...
            if path_exists_no_follow(&pending_path)? {
                ...
                fs::remove_file(&pending_path).map_err(|source| {
                    GenerationStoreError::io(
                        "remove committed activation pending alias",
                        pending_path.clone(),
                        source,
                    )
                })?;
            ...
            #[cfg(test)]
            inject_failure(failpoint, GenerationFailpoint::StartupStagingCleanup)?;
            fs::remove_file(&recovery.path).map_err(|source| {
                GenerationStoreError::io(
        ...
        Ok(GenerationStagingRecoveryReport {
            removed_entries,
            activation_authority_changed: true,
        })
    })();
    recovery.map_err(activation_recovery_requires_reopen)
}

fn recover_disposable_staging(
    staging: &Path,
    activations: &Path,
    budget: &mut AssetLoadBudget,
    #[cfg(test)] failpoint: Option<GenerationFailpoint>,
    ...
    let mut removed_entries = 0_u64;
    let mut activation_authority_uncertain_or_changed = false;
    let recovery = visit_directory_entries_budgeted(staging, budget, |entry, budget| {
        let metadata = metadata_no_follow(&entry.path)?;
        let Some(name) = entry.file_name.to_str() else {
            return Ok(());
        };
        if parse_staging_directory_name(name).is_some()
            || parse_quarantine_directory_name(name).is_some()
        {
            if !metadata.is_dir() {
                return Err(GenerationStoreError::UnsupportedFileType { path: entry.path });
            }
        ...
        if let Some(ordinal) = parse_activation_pending_file_name(name) {
            ...
            let recovery_exists = path_exists_no_follow(&recovery_path)
                .map_err(activation_recovery_requires_reopen)?;
            if recovery_exists {
                return Err(GenerationStoreError::ActivationAuthorityChangedReopenRequired);
            }
            let final_path = activations.join(activation_file_name(ordinal));
            let final_exists =
                path_exists_no_follow(&final_path).map_err(activation_recovery_requires_reopen)?;
            if final_exists {
                // Once a matching ordinal is visible in the final namespace, even a failed alias
                // check leaves this process unable to prove that its in-memory active handle still
                // agrees with persisted authority.
                activation_authority_uncertain_or_changed = true;
                verify_activation_alias(
                    staging,
                    activations,
                    &entry.file_name,
        ...
        if parse_activation_recovery_file_name(name).is_some() {
            return Err(GenerationStoreError::ActivationAuthorityChangedReopenRequired);
        }
    ...
    if let Err(error) = recovery {
        return Err(if activation_authority_uncertain_or_changed {
            activation_recovery_requires_reopen(error)
        } else {
            error
        });
    }
    #[cfg(test)]
    if let Err(error) = inject_failure(failpoint, GenerationFailpoint::StartupStagingDirectorySync)
    {
        return Err(if activation_authority_uncertain_or_changed {
            activation_recovery_requires_reopen(error)
        } else {
    ...
    Ok(GenerationStagingRecoveryReport {
        removed_entries,
        activation_authority_changed: activation_authority_uncertain_or_changed,
    })
}
...
fn recover_owned_staging(
    ...
    let committed =
        recover_committed_activation_authority(staging, activations, budget, failpoint)?;
    merge_recovery_results(
        Ok(committed),
        recover_disposable_staging(staging, activations, budget, failpoint),
    )
}
...
fn next_staging_ordinal(
    ...
    visit_opened_directory_entries_budgeted(staging, opened_staging, budget, |entry, _budget| {
        ...
        let Some(ordinal) = parse_staging_directory_name(name) else {
            return Ok(());
        };
...
fn recover_unreferenced_generation_directories(
    ...
    protected_generation: Option<&Path>,
    budget: &mut AssetLoadBudget,
    #[cfg(test)] failpoint: Option<GenerationFailpoint>,
) -> Result<GenerationStagingRecoveryReport, GenerationStoreError> {
    let mut removed_entries = 0_u64;
    visit_directory_entries_budgeted(generations, budget, |entry, budget| {
        ...
        let protected = protected_generation.is_some_and(|protected| protected == entry.path);
        let remove = !protected && (obsolete || retire_current_generations && current.is_some());
        if !remove {
            return Ok(());
        }
        let metadata = metadata_no_follow(&entry.path)?;
    ...
    Ok(GenerationStagingRecoveryReport {
        removed_entries,
        activation_authority_changed: false,
    })
}
...
fn select_active_generation(
    ...
    budget: &mut AssetLoadBudget,
) -> Result<SelectedGeneration, GenerationStoreError> {
    let selected = if let Some(candidate) = snapshot.candidates.last() {
        ...
        match record {
            Ok(record) => {
                load_completed_generation(generations, opened_generations, record, budget)
                    .map(|generation| SelectedGeneration::Ready(Some(generation)))
            }
            Err(GenerationStoreError::IndexRebuildRequired(required)) => {
                let active = if matches!(
                    GenerationStorageLayout::from_contract(required.storage_contract()),
                    Some(GenerationStorageLayout::OpaqueProjection)
                ) {
                    Some(load_obsolete_projection_generation(
                        generations,
                        opened_generations,
                        required.as_ref(),
                        budget,
...
fn read_activation_record(
    ...
    let record = match decode_generation_head_record(wire, path) {
        Ok(record) => record,
        Err(GenerationStoreError::IndexRebuildRequired(required)) => {
            if required.activation_ordinal != expected_ordinal {
                return Err(GenerationStoreError::ActivationOrdinalMismatch {
                    path: path.to_path_buf(),
                    expected: expected_ordinal,
                    actual: required.activation_ordinal,
                });
            }
            required
                .bootstrap
                .transaction_receipts
                .validate_for_workspace(required.bootstrap.workspace)
                .map_err(|source| GenerationStoreError::ActivationProvenance {
                    path: path.to_path_buf(),
                    source: Box::new(source),
                })?;
            return Err(GenerationStoreError::IndexRebuildRequired(required));
        }
        Err(error) => return Err(error),
    };
...
fn load_obsolete_projection_generation(
    generations: &Path,
    opened_generations: &SecureReadDirectory,
    ...
    load_completed_generation(
        ...
        GenerationHeadRecord {
            contract_version: GENERATION_HEAD_CONTRACT_VERSION,
            ordinal: required.activation_ordinal,
            generation: required.generation,
            generation_storage_contract: required.storage_contract(),
            manifest_digest: required.manifest_digest(),
            workspace: required.bootstrap.workspace(),
            revision: required.bootstrap.actual_revision(),
            desired_revision: Some(required.bootstrap.desired_revision()),
            parent_generation: required.parent_generation(),
            transaction_receipts: required.bootstrap.transaction_receipts().clone(),
        },
        budget,
    )
}
...
fn decode_generation_head_record(
    ...
    let (desired_revision, parent_generation, transaction_receipts) = match contract_version {
        LEGACY_ACTIVATION_CONTRACT_VERSION => {
            require_wire_missing(
                generation_storage_contract,
                &invalid,
                "activation v1 must not declare a generation storage contract",
            )?;
            require_wire_missing(
                desired_revision,
                &invalid,
                "activation v1 must not declare a desired revision",
            )?;
            require_wire_missing(
                parent_generation,
                &invalid,
                "activation v1 must not declare a parent generation",
            )?;
            require_wire_missing(
                transaction_receipts,
                &invalid,
                "activation v1 must not declare transaction receipts",
            )?;
            return Err(GenerationStoreError::IndexRebuildRequired(Box::new(
                IndexRebuildRequired {
                    ...
                    bootstrap: GenerationRebuildBootstrap {
                        ...
                        transaction_receipts: TransactionReceiptWindow::empty(),
                    },
                },
            )));
        }
        REVISIONED_ACTIVATION_CONTRACT_VERSION => {
            require_wire_missing(
                generation_storage_contract,
                &invalid,
                "activation v2 must not declare a generation storage contract",
            )?;
            ...
            require_wire_missing(
                parent_generation,
                &invalid,
                "activation v2 must not declare a parent generation",
            )?;
            require_wire_missing(
                transaction_receipts,
                &invalid,
                "activation v2 must not declare transaction receipts",
            )?;
            return Err(GenerationStoreError::IndexRebuildRequired(Box::new(
                IndexRebuildRequired {
                    reason: IndexRebuildReason::ObsoleteActivationContract {
        ...
        GENERATION_HEAD_CONTRACT_VERSION => {
            ...
            match GenerationStorageLayout::from_contract(generation_storage_contract) {
                Some(GenerationStorageLayout::CurrentSourceStateV5) => (
                    Some(desired_revision),
                    parent_generation,
                    transaction_receipts,
                ),
                Some(
                    GenerationStorageLayout::RebuildOnly
                    | GenerationStorageLayout::OpaqueProjection,
                ) => {
                    return Err(GenerationStoreError::IndexRebuildRequired(Box::new(
                        IndexRebuildRequired {
                            ...
                            activation_ordinal: ordinal,
                            generation,
                            storage_contract: generation_storage_contract,
                            manifest_digest,
                            parent_generation,
                            bootstrap: GenerationRebuildBootstrap {
                                workspace,
...
fn require_wire_missing<T>(
    ...
    match presence {
        ...
        WirePresence::Present(_) => Err(invalid(present_message)),
    }
}
...
fn load_completed_generation(
    ...
    let directory_name = record
        .generation
        .directory_name_for_storage_contract(record.generation_storage_contract);
    ...
    Ok(GenerationSnapshot {
        ...
        generation: record.generation,
        storage_contract: record.generation_storage_contract,
        manifest_digest: completed.manifest_digest,
        desired_revision,
        manifest: completed.manifest,
        activation,
...
#[cfg(test)]
pub(crate) fn rewrite_generation_fixture_as_opaque_storage(
    current_directory: &Path,
    activation_path: &Path,
    generation: SearchGenerationId,
    storage_contract: u16,
    opaque_source_state: &[u8],
) -> PathBuf {
    let obsolete_directory = current_directory
        .with_file_name(generation.directory_name_for_storage_contract(storage_contract));
    fs::rename(current_directory, &obsolete_directory).unwrap();

    let state_directory = obsolete_directory.join(SOURCE_STATE_ARTIFACT_DIRECTORY);
    fs::rename(
        state_directory.join(SOURCE_STATE_FILE),
        state_directory.join("source-state-v4.json"),
    )
    .unwrap();
    rewrite_generation_fixture_source_state(
        &obsolete_directory,
        activation_path,
        "source-state-v4.json",
...
#[cfg(test)]
pub(crate) fn rewrite_generation_fixture_source_state(
    generation_directory: &Path,
    activation_path: &Path,
    ...
    let state_directory = generation_directory.join(SOURCE_STATE_ARTIFACT_DIRECTORY);
    fs::write(state_directory.join(source_state_file), source_state).unwrap();
    let mut manifest: serde_json::Value =
        serde_json::from_slice(&fs::read(generation_directory.join(MANIFEST_FILE)).unwrap())
            .unwrap();
    manifest["artifacts"]["source_state"] =
        serde_json::to_value(measure_artifact_tree(&state_directory).unwrap()).unwrap();
    let manifest_bytes = serde_json::to_vec(&manifest).unwrap();
    fs::write(generation_directory.join(MANIFEST_FILE), &manifest_bytes).unwrap();

    let mut activation: serde_json::Value =
        serde_json::from_slice(&fs::read(activation_path).unwrap()).unwrap();
    if let Some(storage_contract) = storage_contract {
        activation["generation_storage_contract"] = serde_json::json!(storage_contract);
    }
    activation["manifest_digest"] =
        serde_json::to_value(DigestV1::hash_bytes(&manifest_bytes)).unwrap();
    fs::write(activation_path, serde_json::to_vec(&activation).unwrap()).unwrap();
}

#[cfg(test)]
pub(crate) fn rewrite_generation_fixture_as_coupled_storage(
    current_directory: &Path,
    activation_path: &Path,
    generation: SearchGenerationId,
) -> PathBuf {
    let storage_contract = LEGACY_COUPLED_GENERATION_STORAGE_CONTRACT_VERSION;
    let obsolete_directory = current_directory
        .with_file_name(generation.directory_name_for_storage_contract(storage_contract));
    fs::rename(current_directory, &obsolete_directory).unwrap();

    for artifact_directory in [SEARCH_ARTIFACT_DIRECTORY, REFERENCE_ARTIFACT_DIRECTORY] {
        let marker_path = obsolete_directory
            .join(artifact_directory)
            .join("schema-contract.json");
        let mut marker: serde_json::Value =
            serde_json::from_slice(&fs::read(&marker_path).unwrap()).unwrap();
        marker["generation_contract_version"] = serde_json::json!(storage_contract);
        fs::write(marker_path, serde_json::to_vec(&marker).unwrap()).unwrap();
    }

    let manifest_path = obsolete_directory.join(MANIFEST_FILE);
    let mut manifest: serde_json::Value =
        serde_json::from_slice(&fs::read(&manifest_path).unwrap()).unwrap();
    manifest["contract_version"] = serde_json::json!(storage_contract);
    manifest["projection_summary"]["contract_version"] = serde_json::json!(storage_contract);
    manifest["artifacts"]["contract_version"] = serde_json::json!(storage_contract);
    for (key, directory) in [
        ("search", obsolete_directory.join(SEARCH_ARTIFACT_DIRECTORY)),
        (
            "references",
            obsolete_directory.join(REFERENCE_ARTIFACT_DIRECTORY),
        ),
        (
            "source_state",
            obsolete_directory.join(SOURCE_STATE_ARTIFACT_DIRECTORY),
        ),
    ] {
        let mut evidence =
            serde_json::to_value(measure_artifact_tree(&directory).unwrap()).unwrap();
        evidence["contract_version"] = serde_json::json!(storage_contract);
        manifest["artifacts"][key] = evidence;
    }
    let manifest_bytes = serde_json::to_vec(&manifest).unwrap();
...
fn completed_generation_sizes(
    generations: &Path,
    budget: &mut AssetLoadBudget,
) -> Result<Vec<(PathBuf, u64)>, GenerationStoreError> {
    ...
    visit_directory_entries_budgeted(generations, budget, |entry, budget| {
        ...
        let recognized = SearchGenerationId::from_directory_name(name).or_else(|| {
            (1..SEARCH_GENERATION_STORAGE_CONTRACT_VERSION).find_map(|contract| {
                SearchGenerationId::from_directory_name_for_storage_contract(name, contract)
            })
        });
...
fn persisted_read_error(
    ...
    match source {
        ...
        SecureReadError::UnsupportedCaseSensitiveDirectory => {
            GenerationStoreError::UnsupportedPlatform {
                operation: "access per-directory case-sensitive Windows storage",
                path,
            }
        }
...
fn is_obsolete_generation_directory_name(value: &str) -> bool {
    OBSOLETE_GENERATION_DIRECTORY_PREFIXES
        .iter()
...
fn parse_quarantine_directory_name(value: &str) -> Option<u64> {
    ...
    let generation = encoded.get(ACTIVATION_FILE_DIGITS.checked_add(1)?..)?;
    if SearchGenerationId::from_directory_name(generation).is_none()
        && !is_obsolete_generation_directory_name(generation)
    {
        return None;
    }
    ordinal.parse().ok()
...
pub(crate) enum GenerationStoreError {
    ...
    ObsoleteSourceStateStorage {
        actual: u16,
        expected: u16,
    },
    IndexRebuildRequired(Box<IndexRebuildRequired>),
    ...
    ActivationCommitRequiresReopen {
        ordinal: u64,
        source: Box<GenerationStoreError>,
    },
    ActivationRecoveryRequiresReopen {
        source: Box<GenerationStoreError>,
    },
    ActivationOutcomeUnknown,
    ActivationAuthorityChangedReopenRequired,
    OrdinalOverflow,
    SizeOverflow {
        resource: &'static str,
    },
...
impl GenerationStoreError {
    ...
    pub(crate) const fn requires_reopen(&self) -> bool {
        matches!(
            self,
            Self::ActivationRollbackFailed { .. }
                | Self::ActivationCommitRequiresReopen { .. }
                | Self::ActivationRecoveryRequiresReopen { .. }
                | Self::ActivationOutcomeUnknown
                | Self::ActivationAuthorityChangedReopenRequired
        )
    }
}
...
impl fmt::Display for GenerationStoreError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ObsoleteSourceStateStorage { actual, expected } => write!(
                formatter,
                "source-state storage version {actual} is obsolete; expected {expected}"
            ),
            ...
            Self::ActivationCommitRequiresReopen { ordinal, source } => write!(
                formatter,
                "activation {ordinal} has durable recovery authority, but final-head durability was not confirmed ({source}); reopen the generation store before continuing"
            ),
            Self::ActivationRecoveryRequiresReopen { source } => write!(
                formatter,
                "activation recovery changed or may have changed the durable generation authority ({source}); reopen the generation store before continuing"
            ),
            ...
            Self::ActivationAuthorityChangedReopenRequired => formatter.write_str(
                "activation recovery changed the durable generation authority; reopen the generation store before continuing",
            ),
...
impl Error for GenerationStoreError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            Self::ActivationCommitRequiresReopen { source, .. }
            | Self::ActivationRecoveryRequiresReopen { source } => Some(source.as_ref()),
            _ => None,
        }
    }
}
...
mod generation_store_tests {
    ...
    #[test]
    fn quarantine_parser_recognizes_all_rebuildable_generation_contracts() {
        let digest = "0".repeat(DigestV1::BYTE_LEN * 2);
        for version in 1..=SEARCH_GENERATION_STORAGE_CONTRACT_VERSION {
            let name = format!("quarantine-00000000000000000007-generation-v{version}-{digest}");
            assert_eq!(parse_quarantine_directory_name(&name), Some(7));
        }
        assert_eq!(
            parse_quarantine_directory_name(&format!(
                "quarantine-00000000000000000007-generation-v{}-{digest}",
