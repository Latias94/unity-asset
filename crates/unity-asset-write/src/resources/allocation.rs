...
impl<'payload> StreamedResourceExtent<'payload> {
    ...
    pub fn generated(
        ...
        let length = validate_generated_extent(bytes, alignment)?;
        Ok(Self {
    ...
    /// Creates an extent backed by generated bytes with a caller-declared digest.
    ///
    /// This constructor deliberately does not hash `bytes`. It is intended for content-addressed
    /// callers that already validated the digest while establishing ownership of the payload. The
    /// declaration is not an integrity proof: artifact preparation independently hashes the exact
    /// encoded extent under its inspection budget and rejects any mismatch before committing it.
    pub fn generated_declared(
        bytes: &'payload [u8],
        payload_digest: DigestV1,
        alignment: u32,
    ) -> Result<Self, StreamedResourcePlanError> {
...
fn validate_generated_extent(
    bytes: &[u8],
    alignment: u32,
) -> Result<u32, StreamedResourcePlanError> {
    validate_alignment(alignment)?;
    checked_size_usize(bytes.len())
}
