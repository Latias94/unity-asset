...
impl QuerySpec {
    ...
    pub fn type_filter(&self) -> Option<&str> {
        self.type_filter.map(SearchKind::canonical_name)
    }

    pub const fn type_filter_kind(&self) -> Option<SearchKind> {
        self.type_filter
    }
...
macro_rules! define_search_kinds {
    ($($variant:ident => ($canonical:literal, [$($alias:literal),+ $(,)?])),+ $(,)?) => {
        ...
        impl SearchKind {
            ...
            pub const fn filter_aliases(self) -> &'static [&'static str] {
                match self {
                    $(Self::$variant => &[$($alias),+]),+
                }
            }

            pub fn from_filter(raw: &str) -> Option<Self> {
                let normalized = normalize_for_match(raw.trim());
                Self::ALL.iter().copied().find(|kind| {
                    kind.filter_aliases().contains(&normalized.as_str())
                })
            }
        }
    };
...
define_search_kinds! {
    ...
    AnimationClip => ("AnimationClip", ["animation", "animationclip", "anim"]),
    AnimatorController => (
        "AnimatorController",
    ...
    BundleContainer => (
        "BundleContainer",
        ["bundlecontainer", "container", "bundle-container"]
    ),
    File => ("File", ["file"]),
}
...
macro_rules! define_known_diagnostic_codes {
    ($($variant:ident => $wire_code:literal),+ $(,)?) => {
        #[derive(Debug, Clone, Copy, PartialEq, Eq)]
        enum KnownDiagnosticCode {
            $($variant),+
        }
...
define_known_diagnostic_codes! {
    EmptyQuery => "empty_query",
    UnterminatedQuote => "unterminated_quote",
    EmptyQuotedTerm => "empty_quoted_term",
    MissingFilterValue => "missing_filter_value",
    DuplicateFilter => "duplicate_filter",
    UnsupportedTypeFilter => "unsupported_type_filter",
    CandidateLimitExceeded => "candidate_limit_exceeded",
    QueryByteLimitExceeded => "query_byte_limit_exceeded",
    QueryTermLimitExceeded => "query_term_limit_exceeded",
    RetrievalTermLimitExceeded => "retrieval_term_limit_exceeded",
    CandidateFieldByteLimitExceeded => "candidate_field_byte_limit_exceeded",
    CandidateTotalByteLimitExceeded => "candidate_total_byte_limit_exceeded",
    CandidateInputLimitExceeded => "candidate_input_limit_exceeded",
    CandidateEvidenceLimitExceeded => "candidate_evidence_limit_exceeded",
    FuzzyWorkLimitExceeded => "fuzzy_work_limit_exceeded",
    InvalidRetrievalEvidence => "invalid_retrieval_evidence",
    DuplicateCandidateKey => "duplicate_candidate_key",
}
...
impl SearchDiagnostic {
    ...
    pub fn code(&self) -> &str {
        let known = match self {
            Self::EmptyQuery => KnownDiagnosticCode::EmptyQuery,
            Self::UnterminatedQuote { .. } => KnownDiagnosticCode::UnterminatedQuote,
            Self::EmptyQuotedTerm { .. } => KnownDiagnosticCode::EmptyQuotedTerm,
            Self::MissingFilterValue { .. } => KnownDiagnosticCode::MissingFilterValue,
            Self::DuplicateFilter { .. } => KnownDiagnosticCode::DuplicateFilter,
            Self::UnsupportedTypeFilter { .. } => KnownDiagnosticCode::UnsupportedTypeFilter,
            Self::CandidateLimitExceeded { .. } => KnownDiagnosticCode::CandidateLimitExceeded,
            Self::QueryByteLimitExceeded { .. } => KnownDiagnosticCode::QueryByteLimitExceeded,
            Self::QueryTermLimitExceeded { .. } => KnownDiagnosticCode::QueryTermLimitExceeded,
            Self::RetrievalTermLimitExceeded { .. } => {
                KnownDiagnosticCode::RetrievalTermLimitExceeded
            }
            Self::CandidateFieldByteLimitExceeded { .. } => {
                KnownDiagnosticCode::CandidateFieldByteLimitExceeded
            }
            Self::CandidateTotalByteLimitExceeded { .. } => {
                KnownDiagnosticCode::CandidateTotalByteLimitExceeded
            }
            Self::CandidateInputLimitExceeded { .. } => {
                KnownDiagnosticCode::CandidateInputLimitExceeded
            }
            Self::CandidateEvidenceLimitExceeded { .. } => {
                KnownDiagnosticCode::CandidateEvidenceLimitExceeded
            }
            Self::FuzzyWorkLimitExceeded { .. } => KnownDiagnosticCode::FuzzyWorkLimitExceeded,
            Self::InvalidRetrievalEvidence { .. } => KnownDiagnosticCode::InvalidRetrievalEvidence,
            Self::DuplicateCandidateKey { .. } => KnownDiagnosticCode::DuplicateCandidateKey,
...
impl<'de> Deserialize<'de> for SearchDiagnostic {
        ...
        D: Deserializer<'de>,
    {
        ...
        let Some(code) = KnownDiagnosticCode::parse(&wire.code) else {
            return Ok(Self::Unknown {
                contract_version: wire.contract_version,
                code: wire.code,
                severity: wire.severity,
        ...
        let diagnostic = match code {
            KnownDiagnosticCode::EmptyQuery => Self::EmptyQuery,
            KnownDiagnosticCode::UnterminatedQuote => Self::UnterminatedQuote {
                byte_offset: diagnostic_detail(&wire.details, "byte_offset")?,
            },
            KnownDiagnosticCode::EmptyQuotedTerm => Self::EmptyQuotedTerm {
                byte_offset: diagnostic_detail(&wire.details, "byte_offset")?,
            },
            KnownDiagnosticCode::MissingFilterValue => Self::MissingFilterValue {
                field: diagnostic_detail(&wire.details, "field")?,
            },
            KnownDiagnosticCode::DuplicateFilter => Self::DuplicateFilter {
                field: diagnostic_detail(&wire.details, "field")?,
            },
            KnownDiagnosticCode::UnsupportedTypeFilter => Self::UnsupportedTypeFilter {
                value: diagnostic_detail(&wire.details, "value")?,
            },
            KnownDiagnosticCode::CandidateLimitExceeded => Self::CandidateLimitExceeded {
            ...
            KnownDiagnosticCode::QueryByteLimitExceeded => Self::QueryByteLimitExceeded {
                actual: diagnostic_detail(&wire.details, "actual")?,
                limit: diagnostic_detail(&wire.details, "limit")?,
            },
            KnownDiagnosticCode::QueryTermLimitExceeded => Self::QueryTermLimitExceeded {
                actual: diagnostic_detail(&wire.details, "actual")?,
                limit: diagnostic_detail(&wire.details, "limit")?,
            },
            KnownDiagnosticCode::RetrievalTermLimitExceeded => Self::RetrievalTermLimitExceeded {
                actual: diagnostic_detail(&wire.details, "actual")?,
                limit: diagnostic_detail(&wire.details, "limit")?,
            },
            KnownDiagnosticCode::CandidateFieldByteLimitExceeded => {
            ...
            KnownDiagnosticCode::CandidateInputLimitExceeded => Self::CandidateInputLimitExceeded {
                limit: diagnostic_detail(&wire.details, "limit")?,
            },
            KnownDiagnosticCode::CandidateEvidenceLimitExceeded => {
                Self::CandidateEvidenceLimitExceeded {
            ...
            KnownDiagnosticCode::FuzzyWorkLimitExceeded => Self::FuzzyWorkLimitExceeded {
                attempted: diagnostic_detail(&wire.details, "attempted")?,
                limit: diagnostic_detail(&wire.details, "limit")?,
            },
            KnownDiagnosticCode::InvalidRetrievalEvidence => Self::InvalidRetrievalEvidence {
                term_index: diagnostic_detail(&wire.details, "term_index")?,
            },
            KnownDiagnosticCode::DuplicateCandidateKey => Self::DuplicateCandidateKey {
    ...
    for lexeme in lexemes {
        ...
        match field {
            Some((field, value)) if field == "t" || field == "type" => {
                ...
                } else if let Some(kind) = SearchKind::from_filter(value) {
...
fn candidate_matches_filters(query: &QuerySpec, candidate: &CandidateFacts) -> bool {
    if let Some(kind) = query.type_filter
        && normalize_for_match(&candidate.kind) != normalize_for_match(kind.canonical_name())
    {
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn known_diagnostic_codes_are_unique_and_parse_symmetrically() {
        let codes = KnownDiagnosticCode::ALL
            .iter()
            .copied()
            .map(KnownDiagnosticCode::as_str)
            .collect::<Vec<_>>();
        assert_eq!(
            codes.iter().copied().collect::<BTreeSet<_>>().len(),
            codes.len()
        );
        for &code in KnownDiagnosticCode::ALL {
            assert_eq!(KnownDiagnosticCode::parse(code.as_str()), Some(code));
        }
    }
    ...
    fn parse_query_extracts_and_canonicalizes_filters() {
        ...
        assert_eq!(query.type_filter_kind(), Some(SearchKind::Prefab));
