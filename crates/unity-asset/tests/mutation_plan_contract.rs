...
use unity_asset::{
    ...
    ObjectAddress, SourceFingerprint, SourceKind, SourceLocator, SourceMemberId, WorkspaceRevision,
};
...
#[test]
fn all_mutation_primitives_have_stable_canonical_json_and_digest() {
...
fn source_and_payload_coverage_are_exact() {
    ...
    assert!(matches!(
        MutationPlan::new(revision, Vec::new(), Vec::new(), vec![action.clone()]),
...
fn wire_contract_rejects_unknown_tags_and_corrupt_payloads() {
    ...
    assert!(read_json_value(unknown_version).is_err());
...
fn budgeted_json_is_fragmentation_invariant() {
    ...
    assert_eq!(
        contiguous_budget.usage().bytes,
        4 * 1024 + u64::try_from(bytes.len()).unwrap() * 7
    );
}
...
fn serialized_inputs_fail_through_the_caller_budget() {
    let bytes = sample_plan().canonical_json().unwrap();
    let required_bytes = 4 * 1024 + u64::try_from(bytes.len()).unwrap() * 7;
