...
use unity_asset_binary::{
    load_bundle_from_memory, AudioProcessor, AudioCompressionFormat, AudioClip,
};
...
fn test_audio_format_detection() {
    ...
    let formats = [
        ...
        (AudioCompressionFormat::Vorbis, "Ogg Vorbis", "ogg", true, true),
        ...
        (AudioCompressionFormat::VAG, "PlayStation VAG", "vag", true, true),
        (AudioCompressionFormat::XMA, "Xbox XMA", "xma", true, true),
        (AudioCompressionFormat::ATRAC9, "PlayStation ATRAC9", "at9", true, true),
    ];
    ...
    for (format, expected_name, expected_ext, expected_compressed, expected_lossy) in formats {
        ...
        println!("    Extension: {} (expected: {})", info.extension, expected_ext);
        println!("    Compressed: {} (expected: {})", info.compressed, expected_compressed);
...
fn test_audioclip_creation() {
    ...
    for format in formats_to_test {
        ...
        if let unity_asset_binary::audio::types::AudioClipMeta::Modern { compression_format, .. } = &clip.meta {
...
fn test_audio_processing_from_files() {
                ...
                Ok(bundle) => {
                    for asset in &bundle.assets {
                        for asset_object_info in &asset.objects {
                            ...
                            if object_info.class_id == 83 || class_name.contains("Audio") ||
                               class_name == "Object" { // Many audio objects are classified as generic "Object"
                                audio_objects += 1;
                                println!("    Found audio object: {} (ID:{}, PathID:{})", 
    ...
    if audio_objects > 0 {
        ...
        assert!(processing_rate >= 50.0, "Should process at least 50% of audio objects");
    }
...
fn test_audio_format_conversion() {
    ...
    for (format, expected_mime) in mime_tests {
        ...
        println!("  Format {:?} -> MIME: {} (expected: {})", format, mime_type, expected_mime);
    ...
    for format in supported_formats {
        assert!(format.is_supported(), "Format {:?} should be supported", format);
        println!("  ✓ Format {:?} is supported", format);
    }
    
    for format in unsupported_formats {
        assert!(!format.is_supported(), "Format {:?} should not be supported", format);
        println!("  ⚠ Format {:?} is not supported (requires specialized decoder)", format);
    }
...
fn test_advanced_audio_extraction() {
                    ...
                    for asset in &bundle.assets {
                        for asset_object_info in &asset.objects {
                            ...
                            if let Ok(unity_class) = object_info.parse_object() {
                                ...
                                if has_audio_props {
                                    potential_audio += 1;
                                    println!("    Potential audio object: {} (ID:{}, PathID:{})",
                                            class_name, object_info.class_id, object_info.path_id);
                                    ...
                                    if let Some(freq_value) = unity_class.get("m_Frequency") {
                                        if let unity_asset_core::UnityValue::Integer(freq) = freq_value {
                                            println!("      Frequency: {} Hz", freq);
                                        }
                                    }

                                    if let Some(channels_value) = unity_class.get("m_Channels") {
                                        if let unity_asset_core::UnityValue::Integer(channels) = channels_value {
                                            println!("      Channels: {}", channels);
                                        }
                                    }

                                    if let Some(bits_value) = unity_class.get("m_BitsPerSample") {
                                        if let unity_asset_core::UnityValue::Integer(bits) = bits_value {
                                ...
                                if object_info.data.len() > 1024 && class_name == "Object" {
                                    ...
                                    let data_preview = &object_info.data[..32.min(object_info.data.len())];
                                    let has_audio_signature = data_preview.iter().any(|&b| b != 0) &&
                                                            data_preview.len() >= 16;

                                    if has_audio_signature {
                                        println!("    Large binary object (potential audio): {} bytes (ID:{}, PathID:{})",
                                                object_info.data.len(), object_info.class_id, object_info.path_id);
                                    }
                                }
                            }
                        }
    ...
    if analyzed_objects > 0 {
        ...
        assert!(analysis_rate >= 80.0, "Should analyze at least 80% of objects");
    }
