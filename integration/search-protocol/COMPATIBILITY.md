...
Revision 3 changed YAML object selectors from permissive string anchors to canonical nonzero numeric `file_id` values and upgraded the nested YAML ObjectAddress wire version. Revision 4 added the required typed `source_object` reference-hit identity. Revision 5 removes HTML highlight fields, publishes bounded background-operation discovery, and makes `daemon.process_failure` the structured authority for a process-lifetime task failure. The field is always present: `null` means no task failure, while a value binds the draining lifecycle to a closed component and bounded cause. Anchored hits retain exact legacy file-ID mirrors for compatibility, while an unanchored YAML document is represented only by its `document_index` selector and omits `location.file_id` and context `doc_file_id`. Any incompatible JSON shape, semantic invariant, operation, identifier encoding, or frame-limit change requires another business revision after revision 5 is released. Current peers advertise only revision 5 and reject peers that offer only revision 1, 2, 3, or 4 with `no_common_revision`. They must not silently fall back after an accepted revision starts exchanging business frames.
