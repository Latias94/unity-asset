...
- `unity-asset-yaml`: a minimal `GameObject`/`Transform` prefab fixture to regression-test YAML anchor + reference resolution (internal).
...
- `unity-asset` CLI: `deps` to build a best-effort dependency graph (summary/edges/dot/json) via TypeTree PPtr scanning (uses fast parsing when possible; falls back to `Environment` otherwise).
...
- Refactored `unity-asset` `Environment` implementation into smaller modules (`container`/`stream`/`loader`/`object_query`/`pptr`) for maintainability (no intended behavior change).
