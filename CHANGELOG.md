...
- Added typed extraction request, plan, manifest, and report contracts with bundle-container queries, deterministic artifact paths, collision policy, bounded execution, durable manifests, and verified resume.
- Added a revision-bound, consumer-owned search pipeline with transaction-keyed `ChangeSet` handoff, coherent `SearchGeneration` state, strict `/v2` HTTP contracts, authenticated reindexing, and token rotation.
