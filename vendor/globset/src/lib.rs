...
impl GlobSet {
    ...
    pub fn matches_candidate_into(
        ...
        // Global rule precedence only depends on ascending indices; unstable sorting is
        // allocation-free and preserves the same order because duplicate indices are removed.
        into.sort_unstable();
        into.dedup();
    }
        ...
        G: AsRef<Glob>,
    {
        ...
        for (i, p) in it.enumerate() {
            ...
            match MatchStrategy::new(p) {
                ...
                // Keep required-extension patterns in the shared RegexSet. Compiling one
                // meta-regex per extension rule creates an unbounded number of NFA/cache pools.
                MatchStrategy::RequiredExtension(_) | MatchStrategy::Regex => {
                    debug!(
        ...
        let mut strats = Vec::with_capacity(6);
...
mod tests {
    ...
    #[test]
    fn required_extension_patterns_share_the_regex_set_and_keep_global_order() {
        let mut builder = GlobSetBuilder::new();
        builder.add(Glob::new("assets/*/scene?.asset").unwrap());
        builder.add(Glob::new("*.asset").unwrap());
        builder.add(Glob::new("assets/*/scene?.prefab").unwrap());
        builder.add(Glob::new("assets/*/*.asset").unwrap());
        let set = builder.build().unwrap();

        assert_eq!(set.matches("assets/level/scene1.asset"), [0, 1, 3]);
        assert_eq!(set.matches("assets/level/scene2.prefab"), [2]);
    }
